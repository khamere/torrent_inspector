...
const DKOKTO_CAPTURE = (() => {
    ...
    function episodes(left, right) {
        ...
        const rows=keys.map(key=>{
            ...
            else if (!one[0].bytes || !two[0].bytes) state='size unknown';
            else state=one[0].bytes===two[0].bytes?'same size':'different size';
    ...
    function summary(left,right,extra=[]) {
        ...
        const season=episodes(a,b);
        if(season.rows.length) {
            lines.push('', 'Season episodes (loaded filenames only)');
            for(const row of season.rows) lines.push(row.key+': '+row.state, '  A: '+row.a, '  B: '+row.b);
            lines.push('Unnumbered video files: A '+season.unknownA+', B '+season.unknownB);
        }
...
const DKOKTO_REVIEW = (() => {
    ...
    const TABS = ['findings', 'media', 'lookups', 'source', 'notes'];
...
const DKOKTO_REVIEW_UI = (() => {
    ...
    function enhance({dialog, body, title, url, result, refresh, initialTab, external = false}) {
        ...
        for (const [key, label] of [['findings', 'Findings'], ['media', 'MediaInfo'], ['lookups', 'Find releases'], ['source', 'Source check'], ['notes', 'My notes']]) {
        ...
        if (!onTorrent) {
            ...
            for (const pane of [panes.media, panes.source]) pane.append(navigation({url, title}, 'Open torrent and continue review'));
        }
...
const DKOKTO_CAPTURE_UI = (() => {
    ...
    function draw(title) {
        ...
        if (season.rows.length || /\bS\d{1,2}\b/i.test(a.title+' '+b.title)) {
            const section = el('section', undefined, 'dk-compare-episodes');
            section.append(el('h3','Season episodes'));
            section.append(el('p','Matched by episode numbers in the loaded filenames. This checks the listed episodes, not whether the published season is complete.'));
            if (season.rows.length) section.append(table(season.rows.map(row=>({label:row.key+' · '+row.state,a:row.a,b:row.b})),nameA,nameB));
            section.append(el('small','Video files without episode numbers: A '+season.unknownA+' · B '+season.unknownB+'. Captures keep up to 500 files. Samples and subtitles are excluded. Recapture a page after its file list loads. Equal sizes do not prove identical content.'));
        ...
        if (season.rows.length) panel.append(el('p','The reports below are the posted report captured on each page, not a report for every episode. Episode comparisons above use the file lists.'));
        if(!result)
...
const DKOKTO_SOURCE = (() => {
    ...
    function write(data) {
        ...
        try {
            store.setItem(KEY, JSON.stringify({ v: 1, pending: data.pending.slice(0, PENDING), results: Object.fromEntries(results), options: { quiet: data.options?.quiet === true } }));
            return true;
        } catch { return false; }
    }
    ...
    return {
        KEY, PENDING, RESULTS, DAYS, quiet, setQuiet,
    ...
    function renderSection(entry) {
        ...
        if (entry) {
            if (DKOKTO_SOURCE.resultFor(entry)) {
                ...
                wrap.append(el('p', 'These details are remembered for three days. Open the same release on another tracker using Find releases to compare its loaded evidence.'));
            }
        }
