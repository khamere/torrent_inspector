...
const DKOKTO_LISTING = (() => {
    ...
    function ensureBar(){if(!listingPage())return;const root=document.querySelector('.torrent-search__results')||document.querySelector('.torrent-search__component')||document.querySelector('main');if(!root)return;
        ...
        settingsBody.append(label, trackers, groupsButton, manage, backup, log);
        const legend=el('small','✕ Naming error · ✓ Title checks pass · ? Review needed');counts=el('span');counts.setAttribute('role','status');counts.setAttribute('aria-live','polite');bar.append(rules,audit,settings,counts,legend);
...
const DKOKTO_BACKUP_CORE = (() => {
    ...
    const NOT_CARRIED=[
        {key:'dkokto_descriptions_v1',why:'The description-carry prototype is not included by either build and writes no storage in the shipped scripts.'},
    ...
    function gather(store,{when=new Date().toISOString().slice(0,10),withoutNotes=false}={}) {
        ...
        for(const entry of KEYS) {
            ...
            if(withoutNotes) {
                if(entry.key===NOTES_KEY)continue;
...
const DKOKTO_SOURCE_UI = (() => {
    ...
    function reveal() {
        ...
        const wrap = el('div', undefined, 'dk-source-banner');
        wrap.setAttribute('role', 'status');
        wrap.append(el('strong', 'Source check'), looked(), button('Close', () => wrap.remove()), quietBox());
        document.body.append(wrap);
        pin(wrap);
    }
...
const DKOKTO_DETAIL = (() => {
    ...
    function known() {
        ...
        const original=fieldOf('.work__language-link','.meta__language');
    ...
    function schedule(){if(timer)return;knownMemo=null;timer=setTimeout(()=>{timer=null;draw();},250);}
