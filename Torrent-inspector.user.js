        ...
        SITES[host] = {
            ...
            files(doc) {
                return [...doc.querySelectorAll('#file_list_tree .file-name')].slice(0,2000)
                    .map(node => ({path:text(node),bytes:core.bytesOf(text(node.parentElement?.querySelector('.file-size')))}));
            },
            total(doc) {
                const label = [...doc.querySelectorAll('#content-area table td > strong')].find(node => text(node) === 'File Size');
                return core.bytesOf(text(label?.parentElement?.nextElementSibling));
            },
