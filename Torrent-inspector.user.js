...
const DKOKTO_RULES = ((profiles) => {
    ...
    return {list,labelOf,current,choose,forget,check,books,siteFor,baseOf,baseline,resolutions,added,hasRules,settled,BASELINE,KEY,HOSTS,
...
const DKOKTO_REQUESTS = (() => {
    ...
    function show(request,{editable=false}={}) {
        ...
        body.append(el('p',result.term?'Search term: “'+result.term+'” · '+(result.parsed.kind==='unknown'?'kind not recognised':result.parsed.kind+(request.source==='torrent'?' release':' request'))+'. Each link opens that tracker’s own search in a new tab, signed in as you. Nothing is fetched from here.'
